...
class RotasLocalTransacao {
    static async NovasLocalTransacao(req, res) {
        ...
        try {
            const localTransacao = await BD.query(
                `INSERT INTO local_transacao (nome, tipo_local, saldo) VALUES ($1, $2, $3) RETURNING *`,
            [nome, tipo_local, saldo]);
            return res.status(201).json(localTransacao[0]);
        } catch (error) {
            return res.status(500).json({ error:
                 "Erro ao criar LocalTransacao", message: error.message });
    ...
    static async ListarTodas(req, res) {
        try {
            const localTransacao = await BD.query(
                `SELECT * FROM local_transacao`);
            return res.status(200).json(localTransacao.rows); // Retorna lista de LocalTransacao
        } catch (error) {
            return res.status(500).json({ error:
                 "Erro ao listar LocalTransacao", message: error.message });
        }
    }
    static async BuscarId(req, res) {
        ...
        try {
            const localTransacao = await BD.query(
                `SELECT * FROM local_transacao WHERE id_local_transacao = $1`, [id]);
            // Verifica se a LocalTransacao foi encontrada
            if (localTransacao.rows.length === 0) {
                return res.status(404).json({ message:
                     "LocalTransacao não encontrada" });
            }
        ...
        try {
            ...
            if(local_transacao.rows.length === 0){
                return res.status(404).json({erro:
                     "local_transacao não encontrada"})
            }
            return res.status(200).json(local_transacao.rows[0])
        } catch(error){
            return res.status(500).json({error:
                 "Erro ao atualizar dados da local_transacao", error: error.message});
        }
    }
    static async atualizarTodosCampos(req, res) {
        ...
        try {
            const localTransacao = await BD.query(
                `UPDATE local_transacao SET nome = $1, tipo_local = $2, saldo = $3 WHERE id_local_transacao = $4 RETURNING *`,
            [nome, tipo_local, saldo, id]);
            return res.status(200).json(localTransacao.rows[0]);
        }
        catch (error) {
            return res.status(500).json({ error:
                 "Erro ao atualizar LocalTransacao", message: error.message });
        }
    }
    static async deletar(req, res) {
        ...
        try {
            const localTransacao = await BD.query(
                `update local_transacao set ativo = false WHERE id_local_transacao = $1 RETURNING *`, [id]);
            // Verifica se a LocalTransacao foi encontrada
            if (localTransacao.rows.length === 0) {
                return res.status(404).json({ message:
                     "LocalTransacao não encontrada" });
            ...
            return res.status(200).json(localTransacao.rows[0]); // Retorna a LocalTransacao deletada
                ...
                return res.status(500).json({ error: 
                    "Erro ao deletar LocalTransacao", message: error.message });
            }
            }
